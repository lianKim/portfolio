...
`handle` 속성이 있는 라우트만 사이드바 메뉴에 표시된다. 상세 페이지나 생성 페이지처럼 페이지는 존재하지만 메뉴에는 노출하고 싶지 않은 경우 `handle`을 생략하면 된다.
...
다만 지금의 정의 형태로는 중첩 그대로 등록할 수 없었다. 부모가 `Outlet`을 가진 레이아웃이 아니라 일반 페이지 컴포넌트여서, 중첩 등록하면 자식 라우트가 그려질 자리가 없기 때문이다. 그래서 React Router에는 평탄화한 경로로 등록했다. 평탄화 함수는 원래 사이드바 메뉴를 만들 때 전체 경로 목록이 필요해서 구현해둔 것인데, 등록에도 같은 함수를 재사용했다.
...
상세 페이지나 생성 페이지처럼 메뉴에 노출할 필요 없는 라우트는 `handle`을 생략했기 때문에, 이 필터 하나로 메뉴용 라우트만 추려낼 수 있다.
