...
앱을 아예 분리하는 선택지부터 검토했다. 모노레포나 마이크로 프론트엔드 같은 것들을 고려해봤는데, 이런 구조는 독립 배포, 독립 팀, 독립 릴리즈 주기가 필요할 때 가치가 크다. 우리는 한 팀이 하나의 파이프라인으로 함께 릴리즈하고 있었고, 인증과 공통 UI처럼 공유하는 자원도 많았다. 이 규모에서 배포를 쪼개면 오히려 런타임 통합, 공통 의존성 중복, 빌드 복잡도 같은 비용이 얻는 것보다 컸다.
...
함께 진행한 CRA → Vite 전환으로 빌드 시간도 81초에서 34초로 58% 줄일 수 있었다.
