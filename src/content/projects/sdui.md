...
그래서 스타일을 2차원으로 모델링했다. 한 축은 상태(default, active, disabled, error), 다른 축은 base와 의사 선택자(:hover, :focus, ::before 등)다. 상태별 스타일을 우선순위 규칙에 따라 병합해 Emotion의 css로 변환하면, 미디어 쿼리를 포함한 대부분의 CSS 표현이 JSON으로 가능해진다. 
