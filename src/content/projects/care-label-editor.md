...
원인은 요소의 배치 방식이었다. react-grid-layout은 성능을 위해 각 요소를 `top`/`left`가 아니라 `transform: translate`로 배치한다. 이 동작을 그대로 사용하고 있었는데, html2canvas가 CSS transform을 제대로 반영하지 못해, 캡처 시 요소들이 transform이 적용되기 전 위치로 찍힌 것이었다.
